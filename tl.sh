...
# Redirect the output to a temporary file
{
    ...
    # The for loop iterates over directories, excluding specific directories and files
    for d in $(find . -type d \( -not -path "./build*" -and -not -path "./.git*" -and -not -path "./gtest*" -and -not -path "./gtest-demo*" -and -not -path "./workflows*" -and -not -path "./googletest*" \)); do
